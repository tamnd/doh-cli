...
describes what users get. Pick a format with `-o`, or let doh choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
doh <command> -o table   # aligned columns for reading
doh <command> -o jsonl   # one JSON object per line, for piping
doh <command> -o json    # a single JSON array
doh <command> -o csv     # spreadsheet friendly
doh <command> -o tsv     # tab-separated
doh <command> -o url     # just the URL column
doh <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
doh <command> --fields id,title,url
```
...
```bash
doh <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
doh <command>            # a table, because this is a terminal
doh <command> | wc -l    # JSONL, because this is a pipe
```
