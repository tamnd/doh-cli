...
func (Domain) Info() kit.DomainInfo {
	return kit.DomainInfo{
		...
		Identity: kit.Identity{
			Binary: "doh",
			Short:  "A command line for doh.",
			Long: `A command line for doh.
...
func (Domain) Register(app *kit.App) {
	...
	// Resolver op: one record per id, the home of `doh page` and
	// `ant get doh://page/<id>`.
	kit.Handle(app, kit.OpMeta{Name: "page", Group: "read", Single: true,
		Summary: "Fetch a page by path or URL", URIType: "page", Resolver: true,
		Args: []kit.Arg{{Name: "ref", Help: "page path or URL"}}}, getPage)

	// List op: members of a page, the home of `doh links` and `ant ls`.
	// It emits page stubs, so every listed member is itself an addressable
	// doh://page/ URI a host can follow.
...
// Classify turns any accepted input — a bare path or a full doh.com URL —
// into the canonical (type, id), so `ant resolve` and `ant url` touch no network.
func (Domain) Classify(input string) (uriType, id string, err error) {
	id = pagePath(input)
	if id == "" {
		return "", "", errs.Usage("unrecognized doh reference: %q", input)
	}
	return "page", id, nil
}
